<template>
  <div class="mx-auto max-w-7xl px-4 py-6" v-if="event">
    <div class="flex justify-between items-start mb-4 flex-wrap gap-2">
      <header class="flex items-center gap-3 w-full justify-between">
        ...
        <div v-if="!smAndDown" class="flex flex-wrap gap-2 flex-shrink-0">
          ...
          <UTooltip text="Refresh Data">
            <UButton icon="i-lucide-refresh-cw" :loading="eventsStore.isLoading" variant="ghost" @click="refreshEventData()" />
          </UTooltip>
      ...
      <NuxtLink :to="`/admin/events/${eventId}`" class="text-decoration-none hover:underline">
        <div class="flex flex-col gap-1">
          ...
          <p class="text-sm sm:text-base text-muted my-0">{{ event?.description || 'No description provided.' }}</p>
        </div>
      </NuxtLink>
    </div>

    <UTabs v-model="selectedSegmentTab" :items="segmentTabs" />

    <!-- Segment Scores -->
    <div v-for="segment in event?.segments" :key="segment.documentId">
      <UCard v-if="selectedSegmentTab === segment.documentId" class="mt-4">
        <div class="flex items-center gap-2 mb-4">
          <h3 class="font-bold text-lg">{{ segment.name }} Scores</h3>
          <UButton icon="i-lucide-refresh-cw" :loading="eventsStore.isLoading" variant="ghost" size="xs" @click="fetchSegmentScores(segment.documentId)" />
        </div>
    ...
    <!-- Final Rankings -->
    <UCard v-if="selectedSegmentTab === 'final-rankings'" class="mt-4">
      <div class="flex items-center gap-2 mb-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
      ...
      <UCard>
        ...
        <div class="space-y-2">
          <div v-if="!event?.judges?.length" class="text-muted">No judges assigned.</div>
          <div v-for="judge in event?.judges" :key="judge.id" class="flex items-center gap-3 p-2">
      ...
      <!-- Segment Status -->
      <UCard>
        <h3 class="font-bold text-lg mb-2">Segment & Scoring Overview ({{ event?.segments?.length || 0 }})</h3>
        <div v-if="!event?.segments || event.segments.length === 0">
          No segments defined for this event.
        </div>
        <div v-else class="space-y-4">
          <div v-for="segment in event?.segments" :key="segment.id">
            <div class="font-bold flex items-center gap-2">
              <UBadge :color="getSegmentStatusColor(segment.segment_status)" size="xs" class="capitalize">
                {{ segment.segment_status }}
              </UBadge>
              {{ segment.name }} (Weight: {{ segment.weight * 100 }}%)
            </div>
            <div class="ml-4 space-y-1 mt-1">
              <div v-for="category in segment.categories" :key="category.id" class="text-sm">
                <div class="font-medium">{{ category.name }}</div>
                <div class="text-muted">
                  Scoring Progress: {{ getScoringProgress(category, event?.judges || [], event?.scores || []) }}
                </div>
              </div>
      ...
      <UCard>
        ...
        <div class="space-y-4">
          ...
          <USelect
            ...
            label="Segment"
            @update:model-value="(val: any) => { selectedSegmentTab = val; printCategoryId = undefined }"
          />
          <USelect
...
const mobileMenuItems = [[
  { label: 'Print Rankings', icon: 'i-lucide-printer', onSelect: () => showPrintDialog.value = true },
...
function getScoringProgress(category: CategoryData, judges: JudgeData[], eventScores: ScoreData[]) {
  ...
  const judgesWhoScored = new Set(categoryScores.map((s: ScoreData) => s.judge?.id).filter((id) => id))
...
const segmentHeaders = computed(() => {
  ...
  const catHeaders = segmentCategories.value.map((category) => ({
    accessorKey: `category_score_${category.documentId}`,
...
const refreshEventData = async () => { await eventsStore.fetchEvent(eventId) }
...
const currentSegmentCategories = computed(() => {
  ...
  return (selectedSegment.value.categories?.map((cat) => ({ label: cat.name, value: cat.documentId })) || [])
})
...
const fetchRankings = async () => {
  ...
  if (printType.value === 'category') {
    if (!segmentId || !categoryId) { showSnackbar('Please select a segment and a category.', 'warning'); return false }
    ...
    const categoryWeight = selectedSegment.value?.categories?.find((c) => c.documentId === categoryId)?.weight
    printTitle.value = `Category Ranking – ${categoryName} (${Number(categoryWeight) * 100}%)`
  } else if (printType.value === 'segment') {
    if (!segmentId) { showSnackbar('Please select a segment.', 'warning'); return false }
  ...
  try {
    ...
    if (printType.value === 'category') {
      if (printGender.value === 'male' || printGender.value === 'both') maleRankings.value = results.male.filter((p) => p.rank === 1).slice(0, 3)
      if (printGender.value === 'female' || printGender.value === 'both') femaleRankings.value = results.female.filter((p) => p.rank === 1).slice(0, 3)
    } else if (printType.value === 'segment' || printType.value === 'final') {
      if (printGender.value === 'male' || printGender.value === 'both') maleRankings.value = results.male.slice(0, 5)
      if (printGender.value === 'female' || printGender.value === 'both') femaleRankings.value = results.female.slice(0, 5)
    }
    if (!maleRankings.value.length && !femaleRankings.value.length) { showSnackbar('No ranking data found.', 'info'); return false }
    return true
  } catch (err) {
    console.error(err)
...
watch(printType, (val) => {
  ...
  else if (val === 'final') { selectedSegmentTab.value = 'final-rankings'; printCategoryId.value = undefined }
})
